...
/** Registration point for add-on booster packs that share MTGCard's natural loot spawns. */
public final class BoosterPackLootRegistry {
    ...
    public static synchronized void register(Supplier<? extends Item> item, float chance) {
        ...
        float normalized = Math.max(0.0f, Math.min(1.0f, chance));
        Item resolved = item.get();
        if (resolved == null || resolved == Items.AIR || normalized <= 0.0f) return;
        for (Entry entry : ENTRIES) if (entry.item().get() == resolved) return;
        ENTRIES.add(new Entry(item, normalized));
    }
