...
import com.spider.mtgcard.mixin.AbstractContainerScreenAccessor;
...
public final class CardSlotArtRenderer {
    ...
    public static boolean renderSlotArt(AbstractContainerScreen<?> screen, GuiGraphics ctx, Font font, Slot slot) {
        if (screen == null || slot == null || !slot.hasItem()) return false;
        if (vanillaIsShowingTransientStack(screen, slot)) return false;
    ...
    private static boolean vanillaIsShowingTransientStack(AbstractContainerScreen<?> screen, Slot slot) {
        if (!(screen instanceof AbstractContainerScreenAccessor accessor)) return false;

        ItemStack dragging = accessor.mtgcard$getDraggingItem();
        if (slot == accessor.mtgcard$getClickedSlot()
                && dragging != null
                && !dragging.isEmpty()
                && !accessor.mtgcard$isSplittingStack()) {
            return true;
        }

        return accessor.mtgcard$isQuickCrafting()
                && accessor.mtgcard$getQuickCraftSlots().contains(slot)
