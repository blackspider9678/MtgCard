...
public final class SleeveCustomizerScreen extends LegacyContainerScreen<SleeveCustomizerMenu> {
    ...
    private void renderScroller(GuiGraphics g) {
        ...
        g.blitSprite(RenderPipelines.GUI_TEXTURED, isScrollBarActive() ? SCROLLER : SCROLLER_DISABLED, leftPos + 119, y, 12, 15);
    }
    ...
    private void renderGrid(GuiGraphics g, int mouseX, int mouseY) {
        ...
        for (int local = 0; local < COLS * VISIBLE_ROWS; local++) {
            ...
            g.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, x, buttonY, 16, 18);
    ...
    @Override
    protected void renderSlot(GuiGraphics g, Slot slot, int mouseX, int mouseY) {
        if (slot.index == SleeveCustomizerMenu.RESULT_SLOT
                && slot.hasItem()
                && CardItemRegistry.isCard(slot.getItem())
                && menu.selectedIndex() >= 0) {
            int index = menu.selectedIndex();
            Identifier texture = index == 0 ? CardBackTextureRegistry.originalTextureForStackOrDefault(menu.input())
                    : SleeveRegistry.values().get(index - 1).backTexture();
            // renderSlot already runs in GUI-local coordinates; adding leftPos/topPos
            // here would draw a duplicate preview far outside the container.
            g.blit(RenderPipelines.GUI_TEXTURED, texture, slot.x, slot.y,
                    0, 0, 16, 16, 16, 16, 16, 16);
            return;
        }
        super.renderSlot(g, slot, mouseX, mouseY);
    }
